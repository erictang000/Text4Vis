...
def main():
    ...
    broken_folders = [
        "passing_American_football_in_game",
        "passing_American_football_not_in_game",
        "petting_animal_not_cat",
        "shooting_goal_soccer",
        "skiing_not_slalom_or_crosscountry",
        "tying_knot_not_on_a_tie",
        "using_remote_controller_not_gaming",
        "hurling_sport",
        "punching_person_boxing",
        "massaging_persons_head",
    ]
    if args.level == 1:
        ...
        for folder in broken_folders:
            tmp_list = glob.glob(osp.join(args.video_path, folder, '*'))
            src_list += tmp_list
        dest_list = [osp.join(
